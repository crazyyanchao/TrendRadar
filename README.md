...
内置 18 个主流财经媒体信源（时间流展示，自动进入选题终端头条流与状态灯）：

| 类型 | 信源 |
|------|------|
| 官方 RSS | 人民日报（时政，另有 world/society/ywkx 等栏目可加） |
| RSSHub（公共实例，可换自建） | 财联社（电报/深度）、财新、第一财经、央视财经·经济半小时、中国新闻周刊·经济 |
| 自建解析器（`media_sources`） | 每日经济新闻、投资界、经济参考报、新华社·新华网、中经社·新华财经、中国证券报、证监会发布、证券时报·快讯（替代券商中国）、上海证券报、中国基金报、财经网 |

- 已放弃：数据宝、ETF风向标（纯微信公众号，无公开 RSS/API）
- RSSHub 源在 `rss.feeds` 中为完整 URL；自建 RSSHub 实例后仅需替换 URL 前缀，或修改 `rsshub.base_url` 后改用 `media_sources` 的 `type: rsshub`
- `media_sources` 中 `type: site` 源由项目内置解析器抓取（`trendradar/crawler/media/`），失败源只黄灯不阻塞
- 部分源（证券时报/上证报/基金报为 JS 渲染）HTML 解析成功率有限，推荐自建 RSSHub 后用其路由（如 `/stcn/kx`）
